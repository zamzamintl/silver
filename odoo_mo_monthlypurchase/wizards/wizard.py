    ...
    def _get_lines(self):
        ...
        if self.categ_id:
            ...
            query = ("""
             ...
             SELECT purchase_report.product_id as pro_name,purchase_report.category_id as pro_categ,qty_ordered,purchase_report.date_order as odate
                ...
                GROUP BY pro_categ,pro_name,qty_ordered,purchase_report.date_order
                ORDER BY pro_categ,purchase_report.date_order
                                 """.format(date_form=self.date_from, date_to=self.date_to, categ_ids=domain))
        else:
            query = ("""     SELECT purchase_report.product_id as pro_name,purchase_report.category_id as pro_categ,qty_ordered,purchase_report.date_order as odate
                 FROM purchase_report
                join purchase_order  on purchase_order.id = purchase_report.order_id
                join product_product on product_product.id = purchase_report.product_id
                join product_category on product_category.id = purchase_report.category_id
