...
class Report(models.AbstractModel):
    _name = 'report.odoo_mo_top_selling_products.top_temp'
...
class TopSellingProductsXlsx(models.AbstractModel):
    _name = 'report.odoo_mo_top_selling_products.xlsx'
    ...
    def generate_xlsx_report(self, workbook,data,wiz_id):
        ...
        for line in lines:
            ...
            sheet.write(row,1, self.env['product.product'].search([('id','=',line.get('product_id'))],limit=1).name, bold)
