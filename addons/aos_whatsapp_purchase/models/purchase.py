    ...
    def send_whatsapp_automatic(self):
        for purchase in self:
            ...
            if self._get_whatsapp_server() and self._get_whatsapp_server().status == 'authenticated':
                ...
                # }
                #===============================================================             
                template = template_id.generate_email(purchase.id)
                ...
                vals = WhatsappComposeMessage._prepare_mail_message(self.env.user.id, AllchatIDs, purchase and purchase.id,  'purchase.order', body, message_data, subject, partners.ids, attachment_ids, send_message, status)
