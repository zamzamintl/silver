...
class product_double_barcode(models.Model):
    _inherit = 'product.product'

    snd_barcode = fields.Char('Second Barcode', copy=False)
            ...
            if operator in positive_operators:
                ...
                if not product_ids:
                    product_ids = self._search([('barcode', '=', name)] + args, limit=limit,
                                               access_rights_uid=name_get_uid)
                if not product_ids:
                    product_ids = self._search([('snd_barcode', '=', name)] + args, limit=limit,
                                               access_rights_uid=name_get_uid)
...
class product_tmpl_double_barcode(models.Model):
    _inherit = 'product.template'

    snd_barcode = fields.Char('Second Barcode', copy=False, readonly=False)