odoo.define('ecomm_custom.website_sale', function(require) {
'use strict';
...
publicWidget.registry.WebsiteSale = publicWidget.Widget.extend(VariantMixin, {
    ...
    events: _.extend({}, VariantMixin.events || {}, {
        ...
        'change select[name="country_id"]': '_onChangeCountry',
			...
			'change select[name="region_id"]': '_onChangeRegion1',
			'change select[name="district"]': '_onChangeRegion2',
    ...
    _onChangeRegion1:function (ev) {
		console.log("onchange Area");
        if (!this.$('.checkout_autoformat').length) {
            return;
        }
        this._changearea();
    ...
    _changearea: function () {
		...
		console.log("/shop/state_area/"+ $("#region_id").val());
        if (!$("#region_id").val()) {
			console.log("no value");
            return;
        }
        this._rpc({
            route: "/shop/state_area/"+ $("#region_id").val(),
            params: {
                mode: 'shipping',
            },
        }).then(function (data) {
            ...
            var selectdistrict = $("select[name='district']");
            // dont reload state at first loading (done in qweb)
			console.log("1");
			console.log(selectdistrict);
			console.log("2");
			console.log(data.selectdistrict);

            if (selectdistrict.data('init')===0 || selectdistrict.find('option').length===1) {
				console.log('ppppp');
                if (data.district.length) {
                    selectdistrict.html('');
					selectdistrict.append($('<option>').text('')
                            .attr('value','')
                            .attr('data-code', ''));
                    _.each(data.district, function (x) {
						console.log("pppp");
						console.log(x);
	...
	_onChangeRegion2:function (ev) {
		console.log("onchange Area");
        if (!this.$('.checkout_autoformat').length) {
            return;
        }
        this._changedestrict();
    },
    _changedestrict: function () {
		console.log("changedistrict");
		console.log( $("#district").val());
		console.log("/shop/state_district/"+ $("#district").val());
        if (!$("#region_id").val()) {
			console.log("no value");
            return;
        }
        this._rpc({
            route: "/shop/state_district/"+ $("#district").val(),
            params: {
                mode: 'shipping',
            },
        }).then(function (data) {
            ...
            var selectpartition = $("select[name='partition']");
            // dont reload state at first loading (done in qweb)
			console.log("5");
			console.log(selectpartition);
			console.log("6");
			console.log(data.selectpartition);

            if (selectpartition.data('init')===0 || selectpartition.find('option').length===1) {
				console.log('dd');
                if (data.partition.length) {
                    selectpartition.html('');
					selectpartition.append($('<option>').text('')
                            .attr('value','')
                            .attr('data-code', ''));
                    _.each(data.partition, function (x) {
						console.log("dd");
						console.log(x);
                        ...
                        var opt = $('<option>').text(x[1])
                            .attr('value', x[0])
                            .attr('data-code', x[2]);
                        selectpartition.append(opt);
						
                        selectpartition.append(opt);
                    });
                    selectpartition.parent('div').show();
                } else {
                    selectpartition.val('').parent('div').hide();
                }
                selectpartition.data('init', 0);
            } else {
                selectpartition.data('init', 0);
            }
    ...
    _changestate: function () {
        ...
        this._rpc({
			...
			console.log(data.selectregion);
            if (selectregion.data('init')===0 || selectregion.find('option').length===1) {
				 
                if (data.region_id.length) {
                    selectregion.html('');
					selectregion.append($('<option>').text('')
                            .attr('value','')
                            .attr('data-code', ''));
                    _.each(data.region_id, function (x) {
                    ...
                    selectregion.parent('div').show();
                } else {
                    selectregion.val('').parent('div').hide();
					
                }
                selectregion.data('init', 0);
            } else {
                selectregion.data('init', 0);
            }
