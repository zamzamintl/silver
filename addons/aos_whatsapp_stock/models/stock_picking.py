    ...
    def send_whatsapp_automatic(self):
        for pick in self:
            ...
            if self._get_whatsapp_server() and self._get_whatsapp_server().status == 'authenticated':
                ...
                KlikApi.auth()         
                template = template_id.generate_email(pick.id)
                ...
                vals = WhatsappComposeMessage._prepare_mail_message(self.env.user.id, AllchatIDs, pick and pick.id,  'stock.picking', body, message_data, subject, partners.ids, attachment_ids, send_message, status)
