...
class Conversation(models.Model):
            ...
            for user in partner.user_ids:
                if user.has_group('base.group_user'):
                    if user.has_group('prt_mail_messages.group_conversation_own'):
                        ...
                        return 1
                    elif user.has_group('prt_mail_messages.group_conversation_all'):
                        # Has access to Conversations
                        return 1
    ...
    def partner_by_email(self, email_addresses):
        ...
        """
        return self.env['res.partner'].search([('email', 'in', email_addresses)], limit=1, order='id desc')
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if len(to) > 0:
            for email in to.split(","):
                ...
                partner = self.partner_by_email([email_address])
                partner_id = partner.id if partner else self.env['res.partner']. \
                    create({'name': partner_name if partner_name and len(partner_name) > 0 else email_address.split("@")[0],
                            'email': email_address,
                            'category_id': [(4, self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat').id)]
                            if self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat') else False
